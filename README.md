...
- Chrome：`video-batch-assistant-chrome-v1.1.2.zip`
- Edge：`video-batch-assistant-edge-v1.1.2.zip`
...
4. 检查面板中的：
   ...
   - 视频数量；
   - 文件名排序。
5. 根据需要决定是否勾选“发布前人工编辑封面”：
   - 不勾选：视频准备完成后自动发表；
   - 勾选：每个视频发表前暂停，编辑并保存封面后点击“继续”。
6. 点击“开始发布”。
7. 保持页面和浏览器窗口开启，等待批次完成。

### 搜索不到剧名时

自动搜索在等待时间内没有得到唯一匹配项时，脚本不会关闭弹窗，也不会跳过当前视频：

1. 面板变成黄色人工接管状态。
2. 在网站短剧弹窗中修改关键词并人工选择正确短剧。
3. 回到助手面板点击“继续”。
4. 扩展检测到已选短剧后继续设置标注和发表。

如果尚未选择短剧就点击“继续”，助手会继续等待，不会误发。
...
|---|---:|---:|
| 普通页面及弹窗 | 20 秒 | 30 秒 |
| 视频列表 frame | 10 秒 | 20 秒 |
| 视频上传和处理 | 60 秒 | 70 秒 |
| 发表后返回列表 | 60 秒 | 70 秒 |

Edge 的每类等待时限比 Chrome 增加 10 秒。人工选择短剧和人工编辑封面不设超时。
...
每次修改建议至少验证：单视频完整流程、多个视频循环、短剧人工接管、封面开关、停止按钮以及 Chrome/Edge 等待差异。
